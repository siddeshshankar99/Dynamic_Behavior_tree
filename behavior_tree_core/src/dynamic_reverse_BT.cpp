...
void BT::ControlNode::InsertChildAtPosition(unsigned int position, TreeNode* child)
{
    if (position <= children_nodes_.size())
    {
        ...
        // You can initialize the child's state to IDLE or whatever default value you have
...
void InitializeActionPreconditionsMap() {
    // For an action with two preconditions
...
std::pair<BT::ControlNode*, BT::FallbackNode*> ExpandTree(BT::ControlNode* sequence, BT::ConditionNode* cf) {
    ...
    // Check if the action is empty (no preconditions or action)
    if (ABT.empty() || ABT == "") {
...
BT::TreeNode* FindChildByName(BT::ControlNode* node, const std::string& name)
{
    ...
    {
        std::cout << "Checking child: " << child->get_name() << std::endl;  // Debug statement
        if (child->get_name() == name)
        {
            std::cout << "Found child: " << name << std::endl;  // Debug statement
            return child;
        }
    }
    std::cout << "Child not found: " << name << std::endl;  // Debug statement
    return nullptr;
}
...
bool Execute_h(BT::ControlNode* root, int TickPeriod_milliseconds)
{
    ...
    while (ros::ok())
    {
        ...
        else 
        {
            ...
            if (node_button3 && node_button3->get_status() != BT::FAILURE )
            {
            ...
            if (!addedNewCondition)
            {
                ...
                if (node_button2 && node_button2->get_status() != BT::FAILURE)
                {
                    ...
                    if (!node_button1)  // Check if node_button1 doesn't already exist
                    {
                       root->AddChild(new BT::ROSCondition("button1_in_goal_condition_L"));
                       std::cout << "adding button1_in_goal_condition" << std::endl;
...
int main(int argc, char **argv) 
{
    ...
    try
    {
        ...
        //Initial Branch
        sequence1->AddChild(new BT::ROSCondition("button3_in_goal_condition_L"));
