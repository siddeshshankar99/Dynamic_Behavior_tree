  ...
  def execute_cb(self, goal):
    ...
    def pickRobot(loop_executed):
        ...
        while not self.stop_event.is_set() and not loop_executed:
            ...
            attached_objects3 = scene.get_attached_objects([box_name])
            is_attached3 = len(attached_objects3.keys()) > 0 
            print(is_attached3)
            
            is_known3 = box_name in scene.get_known_object_names()
            print(is_known3)
            
            if is_attached3 == True and (is_known3 == False):
                success = True
                rospy.sleep(0.1)
                loop_executed = True 
                ...
                grasps.grasp_pose.pose.orientation.x = -0.924
                grasps.grasp_pose.pose.orientation.y = -0.383         
...
def main():
    rospy.init_node("Pick3_from_goal_R")
    #rospy.init_node("action")
    Pick1_BT(rospy.get_name())
    pose_stamped_callback.pose_stamped = Pose()
    rospy.Subscriber("/button3_pose", PoseStamped, pose_stamped_callback)
    rospy.spin()
