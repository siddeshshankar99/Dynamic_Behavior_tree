  ...
  def execute_cb(self, goal):
    ...
    def pickRobot(loop_executed):
        ...
        while not self.stop_event.is_set() and not loop_executed:
            ...
            attached_objects2 = scene.get_attached_objects([box_name])
            is_attached2 = len(attached_objects2.keys()) > 0 
            print(is_attached2)
            
            is_known2 = box_name in scene.get_known_object_names()
            print(is_known2)
            
            if is_attached2 == True and (is_known2 == False):
                success = True
                rospy.sleep(0.1)
                loop_executed = True 
...
def main():
    rospy.init_node("Pick2_from_goal_R")
    #rospy.init_node("action")
    Pick1_BT(rospy.get_name())
    pose_stamped_callback.pose_stamped = Pose()
    rospy.Subscriber("/button2_pose", PoseStamped, pose_stamped_callback)
    rospy.spin()
