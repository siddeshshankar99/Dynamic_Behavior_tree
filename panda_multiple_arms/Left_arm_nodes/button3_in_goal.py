...
class BTAction(object):
    ...
    def execute_cb(self, goal):
        ...
        while not rospy.is_shutdown() and not loop_executed:
            ...
            if abs(pose_stamped_callback.pose_stamped.position.x - 0.4) <= epsilon_x and abs(pose_stamped_callback.pose_stamped.position.y - (-0.6)) <= epsilon_y and abs(pose_stamped_callback.pose_stamped.position.z - 0.2) <= epsilon_z:
                success = True
...
if __name__ == '__main__':
    rospy.init_node('button3_in_goal_condition_L')
